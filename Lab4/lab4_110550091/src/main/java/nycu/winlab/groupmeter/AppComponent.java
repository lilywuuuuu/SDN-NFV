...
public class AppComponent {
    ...
    private final NameCfgListener cfgListener = new NameCfgListener();
    ...
    private class NameCfgListener implements NetworkConfigListener {
        @Override
        public void event(NetworkConfigEvent event) {
            if ((event.type() == CONFIG_ADDED || event.type() == CONFIG_UPDATED)
                    && event.configClass().equals(HostConfig.class)) {
                HostConfig cfg = cfgService.getConfig(appId, HostConfig.class);
                host1 = ConnectPoint.deviceConnectPoint(cfg.host1());
                host2 = ConnectPoint.deviceConnectPoint(cfg.host2());
                mac1 = MacAddress.valueOf(cfg.mac1());
                mac2 = MacAddress.valueOf(cfg.mac2());
                ip1 = Ip4Address.valueOf(cfg.ip1());
                ip2 = Ip4Address.valueOf(cfg.ip2());
                if (cfg != null) {
                ...
                meterSelectorBuilder.matchEthSrc(mac1);
            ...
            if (ethPkt.getEtherType() == Ethernet.TYPE_ARP) {
                ...
                if (dstIp4Address == ip1.toInt()) {
                    ...
                    log.info("Intent `{}`, port `{}` => `{}`, port `{}` is submitted. ",
                            recConnectPoint.deviceId(),
                            recConnectPoint.port(),
                            host1.deviceId(),
                            host1.port());
                    TrafficSelector.Builder selectorBuilder = DefaultTrafficSelector.builder();
